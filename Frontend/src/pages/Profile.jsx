...
const Profile = () => {
    ...
    const fetchProfile = async () => {
      ...
      try {
        const response = await axios.get(
          `http://localhost:4000/user/${users.upi_id}`
        );
  ...
  }, [users]); // Dependency on `users` context to re-fetch profile when `users` changes
  ...
  const handleAddMoney = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:4000/wallet", {
        upi_id: users.upi_id,
        amount,
      });
      ...
      console.error("Error adding money:", error.response || error);
